...
import { NUM_HINT_LEVELS, generateNumHints, numAdsNeededForNextHint, getUnlockedNumHintIds } from '../utils/numHints';
import { useLanguage } from '../i18n/LanguageContext';

function NumMantleHintSystem({ answer, adsWatchedCount, onWatchAd }) {
  const { lang, t } = useLanguage();

  const unlockedIds = getUnlockedNumHintIds(adsWatchedCount);
  const hints = generateNumHints(answer, unlockedIds, lang);
  const next = numAdsNeededForNextHint(adsWatchedCount);
  const totalHints = NUM_HINT_LEVELS.length;
  ...
  return (
    <div className="w-full max-w-md bg-gradient-to-r from-teal-900 to-cyan-900 p-6 rounded-lg shadow-lg mb-8">
      ...
      {next.hintId && (
        ...
        >
          {t('numWatchAdHint')} ({unlockedIds.length + 1}/{totalHints})
        </button>
      )}
...
export default NumMantleHintSystem;

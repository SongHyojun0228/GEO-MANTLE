...
import { ANIMAL_HINT_LEVELS, generateAnimalHints, animalAdsNeededForNextHint, getUnlockedAnimalHintIds } from '../utils/animalHints';
import { useLanguage } from '../i18n/LanguageContext';

function AnimalMantleHintSystem({ answer, adsWatchedCount, onWatchAd }) {
  const { lang, t } = useLanguage();

  const unlockedIds = getUnlockedAnimalHintIds(adsWatchedCount);
  const hints = generateAnimalHints(answer, unlockedIds, lang);
  const next = animalAdsNeededForNextHint(adsWatchedCount);
  const totalHints = ANIMAL_HINT_LEVELS.length;
  ...
  return (
    <div className="w-full max-w-md bg-gradient-to-r from-teal-900 to-cyan-900 p-6 rounded-lg shadow-lg mb-8">
      ...
      {next.hintId && (
        ...
        >
          {t('animalWatchAdHint')} ({unlockedIds.length + 1}/{totalHints})
        </button>
      )}
...
export default AnimalMantleHintSystem;

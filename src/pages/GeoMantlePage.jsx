  ...
  return (
    <>
      ...
      <article className="w-full max-w-md bg-gray-800 p-6 rounded-lg shadow-lg mb-8 text-gray-300 text-sm space-y-4 leading-relaxed mt-8">
        <h2 className="text-xl font-bold text-teal-400 mb-2">Geo-Mantle: 지리 퍼즐 게임 플레이 가이드</h2>
        <p>
          Geo-Mantle은 매일 새롭게 지정되는 전 세계의 한 '국가'를 추리하여 맞히는 교육적이고 흥미로운 <strong>지리 기반 웹 퍼즐 게임</strong>입니다. 구글 지도와 오픈스트리트맵(OSM) 기반의 좌표 시스템을 이용하여 플레이어가 입력한 국가와 정답 국가 사이의 직선 <strong>거리(km)와 방향(화살표)</strong>을 계산하여 보여줍니다. 이를 통해 자연스럽게 세계 지리와 국가들의 위치를 학습할 수 있습니다.
        </p>
        <p>
          게임의 주요 팁과 전략으로는, 먼저 대륙의 중앙에 위치한 큰 국가(예: 러시아, 브라질, 콩고민주공화국 등)를 첫 번째 추측으로 입력하는 것을 추천합니다. 이렇게 하면 방향 화살표와 힌트를 통해 해당 대륙의 북쪽인지, 혹은 완전히 다른 대륙인지 빠르게 감을 잡을 수 있습니다. 거리가 가까워질수록 유사도(%) 퍼센티지가 높아지며 리스트의 색상이 초록빛이나 붉은빛 등 눈에 띄게 변하므로, 이를 단서로 다음 국가를 유추하세요.
        </p>
        <p>
          또한 게임 내에 마련된 <strong>광고 보기 힌트 시스템</strong>을 적극적으로 활용해보세요. 광고를 짧게 시청함으로써 특정 대륙 힌트를 먼저 해금할 수 있고, 더 나아가 거리 정보를 정확하게 확인할 수 있는 추가 힌트까지 획득하여 퍼즐의 난이도를 낮출 수 있습니다. 이 매일 갱신되는 재미있는 브라우저 게임을 통해 지리 상식의 달인으로 거듭나 보시길 바랍니다!
        </p>
      </article>
    </>
  );

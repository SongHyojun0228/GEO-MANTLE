  ...
  const formatResultsForClipboard = () => {
    ...
    guesses.forEach((item, index) => {
      ...
      if (matchesCountry(item.name, todayAnswerCountry)) {
        // Check against aliases here too
    ...
    showRewardedAd(
      ...
      () => {
        ...
        if (
          ...
          setUnlockedHints(updatedHints);
          alert(
            "🎉 거리 정보 힌트가 해금되었습니다! 이제 모든 추측에서 거리와 유사도(%)를 볼 수 있습니다.",
          );
        }
      },
      // 광고 로드 실패 시
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col items-center p-4">
      ...
      <section className="w-full max-w-md bg-gray-800 p-6 rounded-lg shadow-lg">
        <h2 className="text-2xl font-semibold text-gray-100 mb-4">
          내 추측 ({uniqueGuessesCount}회)
        </h2>
        {guesses.length === 0 ? (
          <div className="text-gray-400">
            <p className="mb-2">
              아직 추측이 없습니다. 첫 번째 국가를 입력해보세요!
            </p>
            {!unlockedHints.distance && (
              <p className="text-sm text-yellow-400 mt-3">
                💡 힌트: 기본적으로 <strong>방향</strong>만 표시됩니다. 광고를
                시청하면 <strong>거리와 유사도(%)</strong>를 볼 수 있어요!
              </p>
            )}
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-700 p-8 rounded-lg shadow-xl text-center relative">
            ...
            <p className="text-gray-200 mb-6">
              축하합니다!{" "}
              <span className="font-semibold">{todayAnswerCountry.name}</span>
              을(를) 맞혔습니다!
            </p>
            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 p-8 rounded-lg shadow-xl text-left relative max-w-lg">
            ...
            <div className="text-gray-200 space-y-3">
              <p>
                매일 새로운 국가가 오늘의 정답으로 선정됩니다. 당신의 목표는 이
                정답 국가를 추측하는 것입니다.
              </p>
              <p>
                입력창에 국가 이름을 입력하고 "추측하기" 버튼을 누르면, 시스템은
                당신이 입력한 국가와 정답 국가 사이의 **거리(km)**와
                **방향(화살표)**을 계산하여 리스트로 보여줍니다.
              </p>
              <p>
                정답에 가까워질수록 리스트의 항목 색깔이 붉게 변하며 유사도를
                %로 표시합니다.
              </p>
              <p>
                추측 기록은 매일 자정(00시)에 초기화됩니다. 그 전까지는 페이지를
                새로고침해도 기록이 유지됩니다.
              </p>
              <p>
                "북한"과 "조선민주주의인민공화국"처럼 여러 이름으로 불리는
                국가들은 모두 같은 국가로 인식됩니다.
              </p>
            </div>
            <button
              onClick={handleToggleInstructionsModal}
